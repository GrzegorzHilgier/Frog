...
namespace Frog.Levels
{
    class Level3 : Level
    {
        public override void Init(List<Player> players, MapInfo mapInfo, Action<DrawableObject> addItemToGame)
        {
            ...
            AddItem(new Wood(Scale, Scale * 6, Scale, height, -2, 0, mapInfo, Players));
            AddItem(new WoodSunk(Scale * 5, Scale * 6, Scale, height, -2, 0, mapInfo, Players));
            AddItem(new Wood(Scale * 12, Scale * 6, Scale, height, -2, 0, mapInfo, Players));

            AddItem(new Wood(Scale, Scale * 7, Scale, height, -1, 0, mapInfo, Players));
            AddItem(new Wood(Scale * 5, Scale * 7, Scale, height, -1, 0, mapInfo, Players));
            AddItem(new Wood(Scale * 12, Scale * 7, Scale, height, -1, 0, mapInfo, Players));
